...
MediaPlayer.rules.DisplaySizeRule = function() {
    "use strict";
    var isMobile, lastSwitchTime = 0, waitToSwitchTime = 1500, testMobile = function() {
    ...
    return {
        ...
        execute: function(context, callback) {
            var self = this, abrController = context.getStreamProcessor().getABRController(), now = new Date().getTime(), mediaInfo = context.getMediaInfo(), mediaType = context.getMediaInfo().type, current = context.getCurrentValue(), switchRequest = new MediaPlayer.rules.SwitchRequest(MediaPlayer.rules.SwitchRequest.prototype.NO_CHANGE, MediaPlayer.rules.SwitchRequest.prototype.WEAK);
            if (now - lastSwitchTime < waitToSwitchTime || !isMobile) {
                callback(switchRequest);
                return;
            }
            var elementWidth = this.videoModel.getElement().offsetWidth, elementHeight = this.videoModel.getElement().offsetHeight;
            if (elementWidth < 600 && elementHeight < 400) {
                var newQuality = abrController.getQualityForBitrate(mediaInfo, (elementWidth / 2 + elementHeight / 2) / 2);
            ...
            if (switchRequest.value !== MediaPlayer.rules.SwitchRequest.prototype.NO_CHANGE && switchRequest.value !== current) {
                self.log("DisplaySizeRule requesting switch to index: ", switchRequest.value, "type: ", mediaType, " Priority: ", switchRequest.priority === MediaPlayer.rules.SwitchRequest.prototype.DEFAULT ? "Default" : switchRequest.priority === MediaPlayer.rules.SwitchRequest.prototype.STRONG ? "Strong" : "Weak");
            }
            lastSwitchTime = now;
            callback(switchRequest);
        },
